...
module DSLs where 
    ...
    {- 
        ...
        This talk:
            What are free monads useful for? (vs what they are) 
            Strategy for writing Embedded DSLs which delay the choice of interpretation
                See related: Tagless Final Algebras and Lawvere Theory/Alg Effects


        What is a DSL?
    
            Def: A set of functions you can use to write domain specific programs

        Ex. SQL for writing DB querries


        Ex. DSL for interacting with a Console
            
            GetLine : M String  
            PutStrLine : String → M Unit

            for some suitable monad M

            
        Ex. DSL for a Key Value Store [String, A]

            GetKey : String → M (Maybe A)
            PutKey : String → A → M Unit
        ...
        Goals: 
            ...
            - Show how to write "Abstract programs" 
                where the DSL operations have not been given a concrete interpretation
            - Show how to give a concrete interpretation to DSL operations
            - Use the concrete interpretation to "compile" the original abstract program
            

            - Define a KeyValue DSL, abstract Key Value programs, concrete interpretations
            - Show how DSLs can be combined 
                use the KeyValue DSL and the Console DSL to make a new LoginServer DSL
            - Show how concrete interpretations can be combined
    ...
    module ConsoleDSL where 
        ...
        -- This can be ignored :) 
        -- A certain type of parameterized types are endofunctors
        -- A constructive proof of this fact can be used to automatically derive `fmap` for some types
        instance
            con-F : Functor ConsoleF
            con-F = record { fmap = λ{ f (PutStrLn s a) → PutStrLn s (f a)
                                     ; f (GetLine m)    → GetLine (f ∘ m) }}
        
        -- ConsoleF describes the Console DSL
        -- Console is the monad we can program in using our DSL
        -- Free takes an Endofunctor F and returns a Free Monad based on F
        ...
        -- Note: These can also be generated
...
module Implementation where
    ...
    module LoginServerMImplementation where 
        ...
        open DSLs.ConsolMImplementation using (Con~>M)
